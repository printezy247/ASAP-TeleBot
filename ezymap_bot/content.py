...
# Native Telegram Payments (via @BotFather -> Payments) providers. code -> display name
# shown to the client (brand names stay in English in both languages, same convention as
# EzyMap/USDT/MT5 elsewhere). Add an entry here (plus its provider token in config.py) to
# offer another BotFather-connected provider the same way.
NATIVE_PAYMENT_PROVIDERS = {
    "ipay88": "iPay88",
    "smartglocal": "Smart Glocal",
}
...
NATIVE_PAYMENT_INVOICE_FAILED_TEXT = {
    "en": (
        "⚠️ Couldn't create a {provider} invoice just now. Please try USDT instead, or "
        "tap below to ask Jack about another way to pay."
    ),
    "my": (
        "⚠️ Tidak dapat cipta invois {provider} buat masa ini. Sila cuba USDT, atau "
        "tekan di bawah untuk tanya Jack cara bayaran lain."
    ),
}
